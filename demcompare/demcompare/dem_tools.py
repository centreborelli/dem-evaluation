...
def compute_and_save_image_plots(
    ...
    import os
    np.save(os.path.join(os.path.dirname(plot_path), "alti_diffs.npy"), dem["image"].data)
...
def difference_histogram(
    ...
    sns.histplot(data=diff, binrange=tuple(np.nanquantile(diff, q=[q_min, q_max])), stat='probability', ax=fig_ax)
